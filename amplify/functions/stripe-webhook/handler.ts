...
export const handler = async (event: any) => {
  ...
  if (stripeEvent.type === 'checkout.session.completed') {
    ...
    const { eventId, distanceId, userId, registrationId } = session.metadata!

    console.log(`Payment completed: registration=${registrationId}, amount=${session.amount_total}`)

    // Update registration status and payment info
    // In production, this would use DynamoDB SDK directly
    // since Amplify Data client isn't available in Lambda easily
